const API_BASE_URL = window.location.origin; // ใช้ same origin สำหรับ API
...
async function handleAnswerSubmit(e) {
    ...
    try {
        const response = await fetch(`${API_BASE_URL}/questions/${currentQuestionId}/answers`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
...
async function loadQuestions() {
    try {
        ...
        console.error('Error:', error);
        questionsList.innerHTML = '<div class="loading error">Error loading questions. Please refresh the page.</div>';
    }
}
    ...
    try {
        ...
        if (response.ok) {
            if (data.data && data.data.length > 0) {
                ...
                answersList.innerHTML = '<div class="loading">No answers yet. Be the first to answer!</div>';
            }
        } else {
            answersList.innerHTML = '<div class="loading error">Error loading answers</div>';
        }
    } catch (error) {
        console.error('Error:', error);
        answersList.innerHTML = '<div class="loading error">Network error loading answers</div>';
    }
}
...
async function handleSearch() {
    ...
    try {
        ...
        const response = await fetch(`${API_BASE_URL}/questions/search?title=${encodeURIComponent(query)}&category=${encodeURIComponent(query)}`);
        const data = await response.json();
        
        if (response.ok && data.data && data.data.length > 0) {
            displayQuestions(data.data);
        } else {
            questionsList.innerHTML = '<div class="loading">No questions found matching your search.</div>';
        }
    } catch (error) {
        console.error('Error:', error);
        questionsList.innerHTML = '<div class="loading error">Error searching questions</div>';
    }
}
